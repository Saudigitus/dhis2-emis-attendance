...
export const CustomDhis2RulesEngine = (props: RulesEngineProps) => {
    ...
    function applyRulesToVariable(variable: any) {
        for (const programRule of newProgramRules.filter(
            ...
            const firstCondition = evaluateExpression(
                        ...
                        if (firstCondition) {
                            const options =
                                getOptionGroups?.filter(
                                    (op) => op.id === programRule.optionGroup
                                )?.[0]?.options || [];
                            variable.options = { optionSet: { options: options } };
                        }
                    }
                    ...
                    if (variable.name === programRule.variable) {
                        if (firstCondition) {
                            if (
                                ...
                                const options =
                                    getOptionGroups
                                        ?.filter((op) => op.id === programRule.optionGroup)?.[0]
                                        ?.options?.slice()
                                        ?.sort(compareStringByLabel) || [];

                                variable.options = {
                                    optionSet: {
                                        options:
                                            variable?.optionSet?.options?.filter(
                                                (obj1: any) =>
                                                    !options.some((obj2) => obj2.value === obj1.value)
                                            ) ||
                                            variable?.initialOptions?.optionSet?.options?.filter(
                                                (obj1: any) =>
                                                    !options.some((obj2) => obj2.value === obj1.value)
                                            ),
                                    },

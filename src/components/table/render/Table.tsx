...
function Table() {
  ...
  const { getData, loading, tableData, getAttendanceData, setTableData } =
    useTableData();
  const headerFieldsState = useRecoilValue(HeaderFieldsState);
  const { selectedDate: selectedDateViewMode } =
    useRecoilValue(SelectedDateState);
  const [selectedDateAddNew, setDateView] = useRecoilState(
    SelectedDateAddNewState
  );
      ...
      if (
        !days.find(
          (x) =>
            x.date ===
            format(
              new Date(selectedDateAddNew?.selectedDate as unknown as Date),
              "yyyy-MM-dd"
            )
        )
      )
